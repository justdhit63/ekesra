// src/components/KelolaPJModal.jsx
import { useState, useEffect } from 'react';
import { supabase } from '../lib/supabaseClient';
import { FaPlus, FaTrash } from 'react-icons/fa';

function KelolaPJModal({ isOpen, onClose, indicator, year, onSave }) {
  ...
  useEffect(() => {
    ...
    const fetchExistingPics = async () => {
      ...
      const { data } = await supabase
        .from('pk_tahunan_penanggung_jawab')
  ...
  const handleSubmit = async (e) => {
    ...
    await supabase.from('pk_tahunan_penanggung_jawab').delete()
      .eq('indikator_sasaran_id', indicator.id)
      .eq('tahun', year);
    
    // Siapkan data baru dari state (filter yang tidak punya profile_id untuk menghindari error)
    ...
    if (newPicData.length > 0) {
      const { error } = await supabase.from('pk_tahunan_penanggung_jawab').insert(newPicData);
      if (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl">
        <h2 className="text-xl font-bold mb-4">Kelola Target PK Tahun {year}</h2>
        ...
        <form onSubmit={handleSubmit}>
            <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                {loading ? <p>Memuat data penanggung jawab...</p> : picList.map((pic, index) => (
