// src/components/IndikatorPJSubKegiatanAccordion.jsx
import { useState, useEffect } from 'react';
import { supabase } from '../lib/supabaseClient';
import { FaChevronDown, FaPlus, FaTrash, FaSave } from 'react-icons/fa';

function IndikatorPJSubKegiatanAccordion({ indicator }) {
  ...
  const fetchPics = async () => {
    ...
    const { data } = await supabase
      .from('penanggung_jawab_indikator_sub_kegiatan')
      .select(`*, profiles(full_name)`)
      .eq('indikator_sub_kegiatan_id', indicator.id);
  ...
  const handleSubmit = async () => {
    setSaving(true);
    await supabase.from('penanggung_jawab_indikator_sub_kegiatan').delete().eq('indikator_sub_kegiatan_id', indicator.id);

    const newPicData = picList.map(pic => ({
      indikator_sub_kegiatan_id: indicator.id,
    ...
    if (newPicData.length > 0) {
      const { error } = await supabase.from('penanggung_jawab_indikator_sub_kegiatan').insert(newPicData);
  ...
  return (
    <div className="bg-white rounded-sm shadow-md">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full bg-green-600 text-white p-3 flex justify-between items-center rounded-t-sm border-b border-gray-200">
              ...
              <table className="table-auto w-full border-collapse border border-gray-200">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="border border-gray-300 p-2">Target 2028</th>
                    <th className="border border-gray-300 p-2">Target 2029</th>
...
export default IndikatorPJSubKegiatanAccordion;