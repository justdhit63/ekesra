...
function Dashboard({ session }) {
    ...
    useEffect(() => {
        // Fungsi untuk mengambil data profil (nama & role)
        const getProfile = async () => {
            ...
            const { data, error } = await supabase
                .from('profiles')
                .select('full_name, role') // Ambil hanya kolom yang kita butuhkan
                .eq('id', user.id)
        ...
        // Fungsi untuk mengambil data perencanaan (jika diperlukan)
        ...
        <div className="min-h-screen">
            ...
            <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                ...
                <div>
                        ...
                        {profile ? (
                            <div className="space-y-2 text-gray-700">
                                ...
                                <p><strong>Role:</strong>
                                    <span className={`ml-2 px-2 py-1 text-xs font-semibold rounded-full ${profile.role === 'admin'
                    ...
                    {/* Konten Halaman Berdasarkan Role */}
                    {profile && profile.role === 'admin' ? (
                        // Tampilan KHUSUS untuk ADMIN
                        <div>
                            <h3 className="text-lg font-semibold text-gray-800 mb-4">Panel Admin</h3>
                            <p>Selamat datang, Admin! Di sini Anda bisa menempatkan panel manajemen pengguna, statistik, atau fitur admin lainnya.</p>
                            {/* Contoh: <UserManagementTable /> */}
                        </div>
                    ) : (
                        // Tampilan untuk PEGAWAI (yang sudah ada sebelumnya)
                        <div>
