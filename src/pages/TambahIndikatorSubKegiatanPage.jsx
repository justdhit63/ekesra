// src/pages/TambahIndikatorSubKegiatanPage.jsx
...
function TambahIndikatorSubKegiatanPage() {
  const { sasaranId } = useParams();
  const navigate = useNavigate();

  const [sasaranSubKegiatan, setSasaranSubKegiatan] = useState(null);
  const [indicators, setIndicators] = useState([{ ...initialIndicatorState }]);
  const [saving, setSaving] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchSasaranSubKegiatan = async () => {
      setLoading(true);
      const { data } = await supabase
        .from('renstra_sasaran_sub_kegiatan')
        .select('deskripsi_sasaran_sub_kegiatan')
        .eq('id', sasaranId)
        .single();
      if (data) {
        setSasaranSubKegiatan(data);
      } else {
        alert("Sasaran Sub Kegiatan tidak ditemukan!");
        navigate('/renstra/sub-kegiatan/sasaran');
      }
      setLoading(false);
    };
    fetchSasaranSubKegiatan();
  }, [sasaranId, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    const indicatorsToInsert = indicators.map(ind => ({ 
      ...ind, 
      sasaran_sub_kegiatan_id: sasaranId 
    }));

    const { error } = await supabase.from('renstra_indikator_sub_kegiatan').insert(indicatorsToInsert);
    ...
    <div>
      ...
      {sasaranSubKegiatan && (
        <p className="text-md text-gray-600 mb-4">
          Untuk Sasaran Sub Kegiatan: <strong>{sasaranSubKegiatan.deskripsi_sasaran_sub_kegiatan}</strong>
        </p>
      )}
        ...
        <div className="space-y-4">
          ...
          {indicators.map((indicator, index) => (
            <div key={index} className="bg-gray-50 p-4 rounded-md border relative">
              ...
              {/* Salin JSX dari form TambahSasaranSubKegiatanPage */}
...
export default TambahIndikatorSubKegiatanPage;