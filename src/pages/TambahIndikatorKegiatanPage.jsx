// src/pages/TambahIndikatorKegiatanPage.jsx
...
function TambahIndikatorKegiatanPage() {
  const { sasaranId } = useParams(); // Ambil ID sasaran dari URL
  const navigate = useNavigate();

  const [sasaranKegiatan, setSasaranKegiatan] = useState(null);
  const [indicators, setIndicators] = useState([{ ...initialIndicatorState }]);
  const [saving, setSaving] = useState(false);
  const [loading, setLoading] = useState(true);

  // Ambil data Sasaran Kegiatan untuk ditampilkan sebagai konteks
  useEffect(() => {
    const fetchSasaranKegiatan = async () => {
      setLoading(true);
      const { data } = await supabase
        .from('renstra_sasaran_kegiatan')
        .select('deskripsi_sasaran_kegiatan')
        .eq('id', sasaranId)
        .single();
      if (data) {
        setSasaranKegiatan(data);
      } else {
        alert("Sasaran Kegiatan tidak ditemukan!");
        navigate('/renstra/kegiatan/sasaran');
      }
      setLoading(false);
    };
    fetchSasaranKegiatan();
  }, [sasaranId, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Siapkan data indikator untuk disimpan dengan ID sasaran yang benar
    const indicatorsToInsert = indicators.map(ind => ({ 
      ...ind, 
      sasaran_kegiatan_id: sasaranId 
    }));

    const { error } = await supabase.from('renstra_indikator_kegiatan').insert(indicatorsToInsert);

    if (error) {
        alert("Gagal menyimpan Indikator: " + error.message);
    } else {
        alert('Indikator baru berhasil ditambahkan!');
        navigate('/renstra/kegiatan/sasaran'); // Kembali ke halaman daftar
    }
    setSaving(false);
  };
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-2">Form Tambah Indikator</h1>
      {sasaranKegiatan && (
        <p className="text-md text-gray-600 mb-4">
          Untuk Sasaran Kegiatan: <strong>{sasaranKegiatan.deskripsi_sasaran_kegiatan}</strong>
        </p>
      )}
      
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-6">
        <div className="space-y-4">
          <h3 className="text-lg font-medium text-gray-800">Indikator Baru</h3>
...
export default TambahIndikatorKegiatanPage;